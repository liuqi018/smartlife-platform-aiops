...
REM 仅启动 SmartLife AIOps 基础设施，不管理 Prometheus/Alertmanager
echo [4/6] 启动 AIOps 基础设施（Redis + Milvus）...
docker ps --format "{{.Names}}" | findstr "milvus-standalone" >nul 2>&1
if not errorlevel 1 (
    echo [信息] Milvus 容器已在运行，确认其余 AIOps 基础设施...
...
start "SmartLife AIOps API" %PYTHON_CMD% -m uvicorn app.main:app --host 0.0.0.0 --port 9900
echo [信息] 等待服务启动（15秒）...
