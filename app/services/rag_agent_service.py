...
使用 langchain_qwq 的 OpenAI-compatible model 原生集成，
支持真正的流式输出和更好的模型适配。
"""
...
class RagAgentService:
    """RAG Agent 服务 - 使用 LangGraph + OpenAI-compatible model 原生集成"""
    ...
    def __init__(self, streaming: bool = True):
        ...
        """
        self.model_name = config.autodl_model or config.rag_model
        ...
        logger.info(f"RAG Agent 服务初始化完成 (OpenAI-compatible model), model={self.model_name}, streaming={streaming}")
    ...
    async def _initialize_agent(self):
        ...
        if mcp_err:
            logger.debug("MCP unavailable; RAG Agent uses local tools only")
            self.mcp_tools = []
        else:
