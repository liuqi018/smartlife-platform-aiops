...
_mcp_health_checked = False
_mcp_cached_tools: list[Union[BaseTool, Any]] = []
_mcp_unavailable_reason: str | None = None
...
async def load_mcp_tools_safe(
    ...
    if _mcp_health_checked:
        return list(_mcp_cached_tools), _mcp_unavailable_reason
    return await initialize_mcp_health(client)


async def initialize_mcp_health(
    client: MultiServerMCPClient | None = None,
) -> tuple[list[Union[BaseTool, Any]], str | None]:
    """Probe MCP once per process and cache tools or degraded status."""
    global _mcp_health_checked, _mcp_cached_tools, _mcp_unavailable_reason
    if _mcp_health_checked:
        return list(_mcp_cached_tools), _mcp_unavailable_reason
    try:
        if client is None:
            client = await get_mcp_client_with_retry()
        _mcp_cached_tools = list(await client.get_tools())
        _mcp_unavailable_reason = None
        logger.info("MCP startup health check passed: tools={}", len(_mcp_cached_tools))
    except BaseException as e:
        _mcp_cached_tools = []
        _mcp_unavailable_reason = f"{type(e).__name__}: {e}"
        logger.info("MCP startup health check unavailable; local-tool fallback enabled: {}", _mcp_unavailable_reason)
    finally:
        _mcp_health_checked = True
    return list(_mcp_cached_tools), _mcp_unavailable_reason
